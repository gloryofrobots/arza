from capy.compile.parse.token_type import *
from capy.compile.parse.indenter import (open_free_layout, open_code_layout,
...
from capy.types import space, api, root, plist, environment
from capy.runtime import error
from capy.misc.strutil import get_line, get_line_for_position
...
TERM_CATCH = [TT_FINALLY]


TERM_BLOCK_START = [TT_COLON]

TERM_FROM_IMPORTED = [TT_IMPORT]

NAME_NODES = [NT_NAME]
IMPORT_NODES = [NT_IMPORT, NT_INCLUDE, NT_INCLUDE_HIDING]
...
INDENTS_TRY = [TT_TRY, TT_FINALLY, TT_CATCH] + INDENTS_CODE
INDENTS_RECEIVE = INDENTS_CODE
INDENTS_USE = [TT_USE, TT_IN]
INDENTS_FUN = [TT_FUN, TT_ASSIGN] + INDENTS_CODE
INDENTS_DECORATOR = [TT_FUN, TT_CLASS] + INDENTS_CODE
INDENTS_CLASS = [TT_CLASS] + INDENTS_CODE
...
def literal(parser, ttype, ntype):
    from capy.compile.parse.callbacks import itself
