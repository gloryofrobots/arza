...
from capy.compile.code.opcode import *
from capy.compile.parse import parser
from capy.compile.parse import token_type as tt
from capy.compile import simplify
from capy.compile.parse import nodes
from capy.compile.parse.nodes import (node_type, imported_name_to_s,
                                      node_first, node_second, node_third, node_fourth,
                                      node_children, is_empty_node)
from capy.compile.parse.node_type import *
from capy.compile.code.source import CodeSource, codeinfo, codeinfo_unknown, SourceInfo
from capy.misc import platform, strutil
...
def _compile_STR(compiler, code, node):
    from capy.misc.strutil import unicode_unescape, decode_str_utf8
...
def _compile_LOOKUP(compiler, code, node):
    ...
    expr_type = node_type(expr)
    if expr_type == NT_TAIL:
        return _emit_TAIL(compiler, code, obj)
    elif expr_type == NT_HEAD:
