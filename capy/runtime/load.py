...
from capy.misc.fs import load_file_content, is_file, join_and_normalise_path
import os
from capy.types import space, api
...
def get_class_filename(process, name):
    raw = api.to_s(name)
    path = "%s.capy" % raw.replace(".", os.sep)
    ...
    if not filename:
        ...
        path = "%s%s%s.capy" % (raw.replace(".", os.sep), os.sep, last_name)
        filename = find_class_file(path, modules_path)
...
def __evaluate_class_env(process, name, filename):
    from capy.builtins.lang import compile_module
