# ************************ OBIN NODES*****************************
NT_GROUP = 0
NT_GOTO = 1
NT_TRUE = 2
NT_FALSE = 3
NT_NIL = 4
NT_INT = 5
NT_FLOAT = 6
NT_STR = 7
NT_CHAR = 8
NT_WILDCARD = 9
NT_NAME = 10
NT_SYMBOL = 11
NT_MAP = 12
NT_LIST = 13
NT_TUPLE = 14
NT_UNIT = 15
NT_COMMA = 16
NT_FUN = 17
NT_CONDITION = 18
NT_TERNARY_CONDITION = 19
NT_WHEN = 20
NT_MATCH = 21
NT_TRY = 22
NT_MODULE = 23
NT_IMPORT = 24
NT_IMPORT_HIDING = 25
NT_IMPORT_FROM = 26
NT_IMPORT_FROM_HIDING = 27
NT_EXPORT = 28
NT_LOAD = 29
NT_TRAIT = 30
NT_GENERIC = 31
NT_SPECIFY = 32
NT_BIND = 33
NT_RETURN = 34
NT_THROW = 35
NT_BREAK = 36
NT_CONTINUE = 37
NT_FOR = 38
NT_WHILE = 39
NT_REST = 40
NT_ASSIGN = 41
NT_CALL = 42
NT_JUXTAPOSITION = 43
NT_CALL_MEMBER = 44
NT_LOOKUP = 45
NT_LOOKUP_SYMBOL = 46
NT_LOOKUP_MODULE = 47
NT_SLICE = 48
NT_RANGE = 49
NT_MODIFY = 50
NT_OF = 51
NT_AS = 52
NT_VAR = 53
NT_LAZY = 54
NT_AND = 55
NT_OR = 56
# ************************ OBIN NODES REPR *****************************
__NT_REPR__ = ["NT_GROUP", "NT_GOTO", "NT_TRUE", "NT_FALSE", "NT_NIL", "NT_INT", "NT_FLOAT", "NT_STR", "NT_CHAR",
               "NT_WILDCARD", "NT_NAME", "NT_SYMBOL", "NT_MAP", "NT_LIST", "NT_TUPLE", "NT_UNIT", "NT_COMMA", "NT_FUN",
               "NT_CONDITION", "NT_TERNARY_CONDITION", "NT_WHEN", "NT_MATCH", "NT_TRY", "NT_MODULE", "NT_IMPORT",
               "NT_IMPORT_HIDING", "NT_IMPORT_FROM", "NT_IMPORT_FROM_HIDING", "NT_EXPORT", "NT_LOAD", "NT_TRAIT",
               "NT_GENERIC", "NT_SPECIFY", "NT_BIND", "NT_RETURN", "NT_THROW", "NT_BREAK", "NT_CONTINUE", "NT_FOR",
               "NT_WHILE", "NT_REST", "NT_ASSIGN", "NT_CALL", "NT_JUXTAPOSITION", "NT_CALL_MEMBER", "NT_LOOKUP",
               "NT_LOOKUP_SYMBOL", "NT_LOOKUP_MODULE", "NT_SLICE", "NT_RANGE", "NT_MODIFY", "NT_OF", "NT_AS", "NT_VAR",
               "NT_LAZY", "NT_AND", "NT_OR", ]


def node_type_to_str(ttype):
    return __NT_REPR__[ttype]
