...
def transform_line(line, index, varname):
    ...
    try:
        ...
        if property_check == start_search_index:
            return line, start_search_index
    except:
        print "not property, working"

    nline = line.replace(BEGIN, "", 1)
    nline = nline.replace("(", "(" + varname + ",", 1)
    return nline, index


def transform(string, varname):
    lines = string.split("\n")
    result = []
    for line in lines:
        index = 0
        ...
        while True:
            try:
                ...
                newline, index = transform_line(newline, index, varname)
            except:
                break
...
print transform(T, "process, compiler")