...
from type_system import AHFunc
...
class sig(Syntax):
    ...
    def __init__(self, signature):
        super(sig, self).__init__("Syntax error in type signature")
    ...
    def __call__(self, fn):
        ...
        return TypedFunc(fn, fn_args, fn_type, self.sig)

class generic(Syntax):
    def __init__(self, typevar):
        super(generic, self).__init__("Syntax error in generic signature")
        self.typevar = typevar

    def __call__(self, type_func):
        if not isinstance(type_func, TypedFunc):
            raise TypeError("TypedFunc expected")

        fn_args = type_func.fn_args
        fn_type = type_func.fn_type
        fn = type_func.func
        sig = type_func.sig
        return AHFunc(self.typevar, fn, fn_args, fn_type, sig)
