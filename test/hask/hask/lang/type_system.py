...
class TypedFunc(Hask):
    ...
    def __init__(self, fn, fn_args, fn_type, signature):
    ...
    def call_fn(self, func, *args, **kwargs):
        ...
        return self.partial_application(result_type, *args, **kwargs)

    def partial_application(self, result_type, *args, **kwargs):
        return TypedFunc(functools.partial(self.func, *args, **kwargs),
                         self.fn_args[len(args):], result_type, self.sig)

    def __call__(self, *args, **kwargs):
        return self.call_fn(self.func, *args, **kwargs)
    ...
    def __mul__(self, fn):
        ...
        * is the function compose operator, equivalent to . in Haskell
        ...
        return TypedFunc(composed_fn, newargs, newtype, self.sig)


class AHFunc(TypedFunc):
    def __init__(self, typevar, fn, fn_args, fn_type, sig):
        super(AHFunc, self).__init__(fn, fn_args, fn_type, sig)
        self.funcs = {}
        self.typevar = typevar

        self.dispatch_index = -1
        for i, arg in enumerate(sig.args):
            if not arg == self.typevar:
                continue
        ...
        if self.dispatch_index == -1:
            raise TypeError("Can not specify generic dispatch argument")


    def register_type(self, t, fn):
        self.funcs[t] = fn

    def register(self, t):
        def _wrap(fn):
            self.register_type(t, fn)
            return fn

        return _wrap

    def partial_application(self, result_type, *args, **kwargs):
        return AHFunc(functools.partial(self.func, *args, **kwargs),
                      self.fn_args[len(args):], result_type, self.sig)

    def __call__(self, *args, **kwargs):
        # the environment contains the type of the function and the types
        # of the arguments
        dispatch_arg = args[self.dispatch_index]
        dispatch_type = type(dispatch_arg)
        if not dispatch_type in self.funcs:
            raise TypeError("Unknown generic function type")

        fn = self.funcs[dispatch_type]
...
def build_ADT(typename, typeargs, data_constructors, to_derive):
    ...
    for i, (dc_name, dc_fields) in enumerate(data_constructors):
        ...
        dcons[i] = TypedFunc(dcons[i], sig_args, make_fn_type(sig_args), sig)
